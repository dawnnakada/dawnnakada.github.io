...
if(typeof title=="string"&&title.length>0){display_title=true;}
...
$el.css({'padding-bottom':padding_bottom}).html(innerHtml.join(''));$thumb=$el.find('.ytp-thumbnail').on('click',function(e){e.preventDefault();if(!$el.hasClass(settings.video_loaded_class)){$el.html('<iframe src="//www.youtube.com/embed/'+id+'?'+youtube_parameters+'&autoplay=1" frameborder="0" allowfullscreen></iframe>').addClass(settings.video_loaded_class);if(typeof settings.callback=='function'){settings.callback.call($el);}}});loadBackgroundImage(id,width,$thumb,youtube_data_url);if((!title&&display_title)||display_duration){if(display_duration)youtube_data_url+=',contentDetails';$.getJSON(youtube_data_url,function(data){var item=data.items[0];$el.find('#lazyYT-title-'+id).text(item.snippet.title);if(display_duration){$el.find('.video-time').text(parseDuration(item.contentDetails.duration,settings)).show();}});}};function loadBackgroundImage(id,width,$thumb,youtube_data_url){var thumb_img,thumb_url,downloadingImage=new Image();if(width==0)width=$thumb.width();var thumbnails=['maxresdefault.jpg','sddefault.jpg','hqdefault.jpg','mqdefault.jpg','default.jpg'];thumb_url=['https://img.youtube.com/vi/',id,'/',thumbnails[0]].join('');downloadingImage.onload=function(data){var naturalWidth=getOnloadDataParam(data,'naturalWidth');var currentSrc=this.src.split("/").pop();var currentIndex=thumbnails.indexOf(currentSrc);if(naturalWidth<150&&currentIndex>-1&&(currentIndex+1)<thumbnails.length){this.src=this.src.replace(currentSrc,thumbnails[currentIndex+1]);}else{setBackgroundImage($thumb,this.src);}};downloadingImage.src=thumb_url;}
...
output.push(('0'+parseInt(durationInSec/60)).slice(-2));output.push(('0'+durationInSec%60).slice(-2));return output.join(':');};$.fn.lazyYT=function(yt_api_key,newSettings){var defaultSettings={yt_api_key:'',youtube_parameters:'rel=0',loading_text:'Loading...',display_title:false,default_ratio:'16:9',display_duration:false,callback:null,video_loaded_class:'lazyYT-video-loaded',container_class:'lazyYT-container'};var settings=$.extend(defaultSettings,newSettings);return this.each(function(){var $el=$(this).addClass(settings.container_class);setUp($el,settings);});};}(jQuery));