...
;(function ($) {
	...
	function loadBackgroundImage(id, width, $thumb, youtube_data_url) {
		...
		var thumbnails = ['maxresdefault.jpg', 'sddefault.jpg', 'hqdefault.jpg', 'mqdefault.jpg', 'default.jpg'];
		// if (width > 640) {
		// 	thumb_img = thumbnails[0];
		// } else if (width > 480) {
		// 	thumb_img = thumbnails[1];
		// } else if (width > 320) {
		// 	thumb_img = thumbnails[2];
		// } else if (width > 120) {
		// 	thumb_img = thumbnails[3];
		// } else if (width == 0) { // sometimes it still might fail on fluid layout
		// 	thumb_img = thumbnails[2];
		// } else {
		// 	thumb_img = thumbnails[4];
		// }

		thumb_url = ['https://img.youtube.com/vi/', id, '/', thumbnails[0]].join('');

		downloadingImage.onload = function(data) {
			// search for youtube thumbnail
			var naturalWidth = getOnloadDataParam(data, 'naturalWidth');
			var currentSrc = this.src.split("/").pop();
			var currentIndex = thumbnails.indexOf(currentSrc);
			if (naturalWidth < 150 && currentIndex > -1 && (currentIndex + 1) < thumbnails.length) {
				this.src = this.src.replace(currentSrc, thumbnails[currentIndex + 1]);
			} else {
	...
	function setBackgroundImage($thumb, url) {
		$thumb.css({'background-image': ['url(', url, ')'].join('')}).addClass('lazyYT-image-loaded');
	}
